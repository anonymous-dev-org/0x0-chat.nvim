...
local function bind_buffer(run, bufnr)
  ...
  if vim.b[bufnr].zeroxzero_run_review_bound then
    return
  end
  vim.b[bufnr].zeroxzero_run_review_bound = true
...
local function attach_diffview_keymaps(run)
  local group = vim.api.nvim_create_augroup("zeroxzero_run_review_" .. run.run_id, { clear = true })
