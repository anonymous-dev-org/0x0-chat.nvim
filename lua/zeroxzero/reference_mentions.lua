...
function M.parse(input, cwd)
  ...
  while pos <= #input do
    ...
    if token == "recent" or token:match("^recent%:%d+$") then
      ...
      if not seen[key] then
        seen[key] = true
        table.insert(mentions, { raw = "@" .. token, type = "recent", count = n })
      end
      goto continue
    end
    ...
    if token == "repomap" then
      if not seen["repomap"] then
        seen["repomap"] = true
        table.insert(mentions, { raw = "@" .. token, type = "repomap" })
      end
      goto continue
    end
    ...
    if token == "test-output" or token == "test_output" then
      if not seen["test-output"] then
        ...
        table.insert(mentions, { raw = "@" .. token, type = "test_output" })
      end
      goto continue
    end
...
local function source_position()
  ...
  for _, win in ipairs(vim.api.nvim_tabpage_list_wins(current_tab)) do
    ...
    if
      ft ~= "zeroxzero-chat-input"
      and ft ~= "zeroxzero-chat-files"
      and ft ~= "zeroxzero-inline-edit-input"
      and ft ~= "zeroxzero-inline-ask-input"
      and bt ~= "nofile"
    then
...
local function format_lsp_block(mention, cwd)
  local LSP = require("zeroxzero.context.lsp")
...
function M.to_prompt_blocks(input, cwd)
  ...
  for _, mention in ipairs(M.parse(input, cwd)) do
    ...
    elseif mention.type == "repomap" then
      local RepoMap = require("zeroxzero.context.repo_map")
