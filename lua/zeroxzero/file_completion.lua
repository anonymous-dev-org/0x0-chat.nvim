...
local ns = vim.api.nvim_create_namespace("zeroxzero_file_completion")
...
local function current_mention_prefix()
  ...
  if start_col > 1 and not before:sub(start_col - 1, start_col - 1):match("%s") then
    return nil, nil
  end
...
local function ensure_menu_buf()
  ...
  vim.bo[bufnr].filetype = "zeroxzero-chat-files"
...
function M.completefunc(findstart, base)
  ...
  local matches = {}
  for _, item in ipairs(matches_for_token(base)) do
